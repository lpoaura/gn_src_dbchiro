...
CREATE OR REPLACE FUNCTION src_dbchirogcra.fct_tri_c_upsert_dbchiro_observation_to_geonature() RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE the_record RECORD;
the_id_synthese INTEGER;
BEGIN
SELECT r.* INTO the_record
FROM (
        WITH obs AS (
            SELECT ss.*,
                taxref.lb_nom
            FROM src_dbchirogcra.sights_sighting ss
        ...
        SELECT DISTINCT obs.uuid AS unique_id_sinp,
            ...
            ref_nomenclatures.get_id_nomenclature('ETA_BIO', '0') AS id_nomenclature_bio_condition,
...
,
            ...
            obs.timestamp_update AS meta_update_date -- Extended synthese part
,
        ...
        FROM obs
            ...
            JOIN src_dbchirogcra.sights_session ses ON obs.session_id = ses.id_session --                          LEFT JOIN sobs ON sobs.id_sighting = obs.id_sighting
            JOIN src_dbchirogcra.accounts_profile a ON ses.main_observer_id = a.id
            ...
            (
                ...
                WHERE name_source LIKE 'dbChiroGCRA'
            ) AS source
    ) AS r;
IF found THEN RAISE NOTICE '<fct_tri_c_upsert_dbchiro_observation_to_geonature> UPSERT sighting %',
new.id_sighting;
...
ELSE RAISE NOTICE '<fct_tri_c_upsert_dbchiro_observation_to_geonature> sighting % excluded from UPSERT',
new.id_sighting;
END IF;
RETURN new;
END;
...
INSERT
    OR
UPDATE ON src_dbchirogcra.sights_sighting FOR EACH ROW EXECUTE PROCEDURE src_dbchirogcra.fct_tri_c_upsert_dbchiro_observation_to_geonature();
/* DELETE Trigger */
DROP FUNCTION IF EXISTS src_dbchirogcra.fct_tri_c_delete_dbchiro_observation_from_geonature() CASCADE;
CREATE OR REPLACE FUNCTION src_dbchirogcra.fct_tri_c_delete_dbchiro_observation_from_geonature() RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE the_id_synthese INT;
the_unique_id_sinp UUID;
BEGIN RAISE NOTICE '<fct_tri_c_delete_dbchiro_observation_from_geonature> Delete sighting % with uuid %',
old.id_sighting,
old.uuid;
...
AFTER DELETE ON src_dbchirogcra.sights_sighting FOR EACH ROW EXECUTE PROCEDURE src_dbchirogcra.fct_tri_c_delete_dbchiro_observation_from_geonature();